...
import { Prescricao } from '@/types/telemetry';
...
export default async function PrescricaoPage({ params }: { params: { id: string } }) {
  ...
  try {
    ...
    const response = await fetch(
      `/api/prescricoes/lista/?maquina_id=${encodeURIComponent(maquinaId)}`,
      {
        cache: 'no-store',
    ...
    <AppShell active="/dashboard" eyebrow="Manutencao" title="Prescricoes">
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-50">Prescricoes para {maquinaId}</h1>
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <h2 className="text-lg font-semibold text-slate-50">{prescricao.titulo}</h2>
                    ...
                    <p className="text-sm text-slate-400">
                      ...
                      {new Date(prescricao.data_geracao).toLocaleDateString('pt-BR')} 
                      {new Date(prescricao.data_geracao).toLocaleTimeString('pt-BR')}
                    </p>
                  </div>
                  {/* Badge de status */}
                  <span className={`px-3 py-1 rounded text-xs font-semibold ${
                    ...
                    {prescricao.status === 'pendente' ? 'PENDENTE' : 
                     prescricao.status === 'concluida' ? 'CONCLUIDA' : 'CANCELADA'}
                  </span>
                </div>
