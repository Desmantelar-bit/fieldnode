...
export default function HarvestersPage() {
  ...
  useEffect(() => {
    telemetryService
      .getLatestReadings()
      .then(setReadings)
      .catch(() => setError("A API de ultimas leituras nao respondeu. Sem drama teatral: confira o Django e tente de novo."));
  }, []);

  if (error) {
    return (
      <AppShell active="/colheitadeiras" eyebrow="Operacao" title="Maquinas agricolas">
        <ErrorState title="Nao consegui carregar as maquinas." message={error} />
      </AppShell>
    );
  }
  ...
  const critical = readings.filter((r) => r.status_risco?.rotuloRisco === "CRITICO").length;
  const warning = readings.filter((r) => r.status_risco?.rotuloRisco === "ATENCAO").length;
  const averageTemp = readings.length
