...
const getIconByStatus = (status: string) => {
  const color = status === 'operando' ? '#10b981' : status === 'parada' ? '#f59e0b' : '#ef4444';
...
export default function MapClient({ positions, isMobile }: { positions: MachinePosition[]; isMobile: boolean }) {
  return (
    <div className="min-h-[50vh] h-[calc(100vh-5.5rem)] w-full sm:h-[calc(100vh-5rem)]">
      <MapContainer
        center={positions.length ? [positions[0].lat, positions[0].lng] : [-15.793889, -47.882778]}
      ...
      >
        ...
        {positions.map((machine) => (
          <Marker key={machine.id} position={[machine.lat, machine.lng]} icon={getIconByStatus(machine.status)}>
            <Popup>
              <strong>Máquina: {machine.modelo}</strong>
              <br />
              <strong>Status:</strong> {machine.status === 'operando' ? 'Operando' : machine.status === 'parada' ? 'Parada' : 'Offline'}
              <br />
